...
public class AIOrchestratorService {
    ...
    public AIOrchestratorService(AIResponseService aiResponseService, ChatClient chatClient, ChatMemory chatMemory, VectorStore vectorStore) {
    ...
    public void registerRequest(EventDTO requestDTO) {
        ...
        String response = chatClient.prompt()
                .system(s -> s.text("The current chatroom id is {chatroom}, few facts from this chatroom or about this user: {memory}")
                        ...
                        .param("memory", longTermMemory.isEmpty() ? "No prior facts known." : longTermMemory))
...
//                .advisors(VectorStoreChatMemoryAdvisor.builder(vectorStore).defaultTopK(10).build())
                .advisors(advisorSpec -> advisorSpec.param(CONVERSATION_ID, requestDTO.getChatroomId()))
                .call()
                .content();
